...
| 画面ID | 画面名 | 概要 |
|---|---|---|
| SCR-060 | Show Card List | 4カテゴリ（食事・体調・交通・支払い）を2×2グリッド表示 |
| SCR-061 | Show Card Detail | カテゴリ内カード一覧。食事はアレルゲン選択式・交通は都市→駅→路線の3段階選択でフレーズ自動生成 |
| SCR-062 | Show Card Full | 選択カードをフルスクリーン表示（日本語大・英語小・カテゴリカラー背景）。タップで閉じる |

#### 📅 Plan タブ（v1.2）

| 画面ID | 画面名 | 概要 |
|---|---|---|
| SCR-070 | Plan Home | 上部セグメント「Planner / Calendar」で切り替え |
| SCR-071 | Trip Planner | Wishlist・ルート・日程の3ビューをセグメントで切り替え。Wishlist はカテゴリ・位置情報付き、URL タイトル自動取得（Edge Function）。ルートはドラッグ&ドロップ＋Google Maps エクスポート。日程は午前・午後・夜の3ブロック |
| SCR-072 | Season Calendar | 月別リストで自然イベント・祭り・混雑ピーク・時間帯アドバイスを表示。カテゴリ＋都市フィルター（選択状態を AsyncStorage に保持）。静的バンドル |

#### 📔 Journal タブ拡張（v1.2）

| 画面ID | 画面名 | 概要 |
|---|---|---|
| SCR-040 | Journal List | （既存）活動記録の一覧 |
| SCR-041 | Journal Edit | （拡張）`insights` フィールド追加（気づき・学びメモ） |
| SCR-042 | Trip Summary | （既存）旅のまとめ |
| SCR-043 | Memory | 写真アルバム＋食べたものログ。日付グループ＋カテゴリフィルター（Food / Place / Experience）。カメラロール＋アプリ内撮影対応。食事は店名・感想を任意入力 |
| SCR-044 | Stats & Badges | 旅の統計（訪問スポット数・Journal 件数・カテゴリ別内訳・合計支出）＋バッジ一覧（約10件・未獲得グレーアウト・獲得時ポップアップ）。アプリ累計で蓄積 |
...
|------|-----------|---------|
| 2026-05-12 | 1.0 | 初版作成 |
| 2026-06-14 | 1.2 | Show タブ・Plan タブ・Journal タブ拡張（Memory・Stats & Badges）の画面追加 |
